...
if __name__ == "__main__":
    ...
    # initailize train loss and validation loss for draw
    train_loss_draw = []
    val_loss_draw = []

    # define a list that contains multiple hidden units
    hidden_units = [5, 20, 50, 100, 200]
    # loop through the hidden units
    for n_hid in hidden_units:
        ...
        # get the average training cross entropy loss
        # get the sum of all training losses
        sum_train_loss = 0
        for i in range(len(train_losses)):
            sum_train_loss += train_losses[i]
        # divide by the number of training losses
        train_loss = sum_train_loss / len(train_losses)
        # put the training loss into train_loss_draw
        train_loss_draw.append(train_loss)

        # get the average validation cross entropy loss
        # get the sum of all validation losses
        # note that validation is test set here in the code
        sum_vali_loss = 0
        for i in range(len(test_losses)):
            sum_vali_loss += test_losses[i]
        # divide by the number of validation losses
        vali_loss = sum_vali_loss / len(test_losses)
        # put the validation loss into val_loss_draw
        val_loss_draw.append(vali_loss)

    # draw the plot
    plt.plot(hidden_units, train_loss_draw, label="Training Loss")
    plt.plot(hidden_units, val_loss_draw, label="Validation Loss")
    # put dots on the lines where hidden unit is 5, 20, 50, 100, 200
    # match the dot color with lines
    plt.plot(5, train_loss_draw[0], 'o', color='tab:blue')
    plt.plot(5, val_loss_draw[0], 'o', color='tab:orange')
    plt.plot(20, train_loss_draw[1], 'o', color='tab:blue')
    plt.plot(20, val_loss_draw[1], 'o', color='tab:orange')
    plt.plot(50, train_loss_draw[2], 'o', color='tab:blue')
    plt.plot(50, val_loss_draw[2], 'o', color='tab:orange')
    plt.plot(100, train_loss_draw[3], 'o', color='tab:blue')
    plt.plot(100, val_loss_draw[3], 'o', color='tab:orange')
    plt.plot(200, train_loss_draw[4], 'o', color='tab:blue')
    plt.plot(200, val_loss_draw[4], 'o', color='tab:orange')
    # set the x and y labels
    plt.xlabel("Number of Hidden Units")
    plt.ylabel("Cross Entropy Loss")
    plt.title("Training and Validation Losses vs. Number of Hidden Units")
    plt.legend()
    # save the plot
    plt.savefig("loss_vs_hidden_units.png")
